...
- [Which notebook this was built from](#which-notebook-this-was-built-from)
...
├── interactions.py               DDInter 2.0 drug-drug interaction lookup
├── rxnorm.py                     optional brand→generic normalisation
├── app.py                        shim → streamlit_app.py (legacy entry point)
│
├── requirements.txt
...
│   ├── doc_embeddings.npy        cached embeddings (git-ignored, auto-rebuilt)
...
│
└── PhARMA_RAG_*.ipynb            the three source notebooks (all still run)
```
...
## Which notebook this was built from

All three notebooks were read **and executed end to end** (0 failed cells):
